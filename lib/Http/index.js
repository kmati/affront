...
/*******************
 * _baseRequestHeaders is an object that contains request headers that are to be issued with every HTTP request
 * if it is provided. If, however, _baseRequestHeaders is falsy then no extra HTTP headers are added except what
 * are passed via the headers argument of the methods.
 */
let _baseRequestHeaders = null;

// Combines the base request headers with other headers
// headers: An object of request headers
//	(keys = request header names, values = request header values)
// Returns: The resulting headers
function combineHeadersStartingWithBase(headers) {
	if (!headers) {
		// no headers so return the base headers regardless
		return _baseRequestHeaders;
	}
	if (!_baseRequestHeaders) {
		// no base headers so return the headers regardless
		return headers;
	}

	let obj = {};

	// clone the base headers
	for (let key in _baseRequestHeaders) {
		obj[key] = _baseRequestHeaders[key];
	}

	// overwrite with the headers
	for (let key in headers) {
		obj[key] = headers[key];
	}

	// return the resulting headers
	return obj;
}
...
export const Http = {
	...
	get BaseRequestHeaders() {
		return _baseRequestHeaders;
	},

	// Sets the base request headers
	set BaseRequestHeaders(val) {
		if (typeof val !== 'object' || val instanceof Array) {
			throw new InvalidArgumentError('Cannot set the BaseRequestHeaders because the value passed in is not a non-Array object');
		}
		_baseRequestHeaders = val;
	},
	...
	get(key, url, callback, headers) {
		...
		request('GET', applyBaseUri(url), null, (err, value) => {
			...
			callback(null, value);
		}, combineHeadersStartingWithBase(headers));
	},
