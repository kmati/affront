 ...
 */
 // BUG: currentVersion is NOT always the last element in versions! You need to manually track currentVersion
 //	=> if you navigate Back in browser then currentVersion = currentVersion - 1
 //	=> but then after you navigate Forward in browser then currentVersion = currentVersion + 1!!!! 
 // ---> currently the bug is that the Back navigate removes the last version and so the Forward does NOT show the correct state!!!
...
class Storage {
	...
	sendNotificationsForCurrentVersion() {
